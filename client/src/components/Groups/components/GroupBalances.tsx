...
interface GroupBalancesProps {
  ...
  onSettle?: (targetId: number, targetName: string, settleAmount: number) => void;
  onRemind?: (targetId: number, targetName: string, amount: number) => void;
}

const getInitials = (name: string) => name ? name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) : 'U';

export default function GroupBalances({ settlements, currentUserId, onSettle, onRemind }: GroupBalancesProps) {
    ...
    <div className="space-y-4 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col gap-3">
        {settlements.settlements.map((s, idx) => {
          const isMeOwe = s.from_id === currentUserId;
          const isMeOwed = s.to_id === currentUserId;
            ...
            >
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center -space-x-2">
                    <div className="w-10 h-10 rounded-full bg-slate-200 dark:bg-white/10 border-2 border-white dark:border-[#1a1a1a] flex items-center justify-center text-xs font-bold text-slate-600 dark:text-slate-300 z-10">
                      {isMeOwe ? 'You' : getInitials(s.from_name)}
                    </div>
                    <div className="w-10 h-10 rounded-full bg-slate-300 dark:bg-white/20 border-2 border-white dark:border-[#1a1a1a] flex items-center justify-center text-xs font-bold text-slate-700 dark:text-white z-0">
                      {isMeOwed ? 'You' : getInitials(s.to_name)}
                    </div>
                  </div>
                  
                  <div className="flex flex-col">
                    <span className="text-sm text-slate-800 dark:text-slate-200 leading-tight">
                      <strong className="text-slate-900 dark:text-white">{isMeOwe ? 'You' : s.from_name}</strong> owes <strong className="text-slate-900 dark:text-white">{isMeOwed ? 'You' : s.to_name}</strong>
                    </span>
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                  ...
                  {isMeOwe && onSettle && (
                    <button 
                      onClick={() => onSettle(s.to_id, s.to_name, s.amount)}
                  ...
                  {isMeOwed && onRemind && (
                    <button 
                      onClick={() => onRemind(s.from_id, s.from_name, s.amount)}
