...
async def run_daily_nudges(target_rule: str = "all"):
    ...
    try:
        ...
        cursor.execute("SELECT id, mobile, created_at FROM users WHERE is_verified = TRUE AND mobile IS NOT NULL")
        ...
        for user in users:
            ...
            template_to_send, trigger_reason, variables = None, "", []
            ...
            for rule in active_rules:
                ...
                if rule['rule_type'] == 'monthly' and is_month_end and now.hour == 22:
                    ...
                    if rule['hours_min'] <= hours_inactive < rule['hours_max']:
                        ...
                        template_to_send, trigger_reason = rule['template_name'], rule['rule_name']
                        variables = [f"{month_total:g}"] # Default fallback for standard daily nudges
                        break
